...
const BilibanBackupSync = {
  ...
  async archivePendingData() {
    ...
    if (active && active.dir === this.ARCHIVE_DIR && active.meta && active.meta.sizeBytes < thresholdBytes) {
      const activeData = await this.readDataFile(active.name, active.dir);
      if (activeData && Array.isArray(activeData.events)) {
        const mergedEvents = activeData.events.slice();
        while (queue.length > 0) {
          const ev = queue[0];
          const est = JSON.stringify({ events: mergedEvents.concat(ev) }).length;
          if (est > thresholdBytes) break;
          mergedEvents.push(ev);
          queue.shift();
        }
        if (mergedEvents.length > activeData.events.length) {
          chunks.push({ id: active.name, events: mergedEvents });
        }
      }
    }
    ...
    while (queue.length > 0) {
      ...
      if (chunkEvents.length > 0) {
        chunks.push({ id: 'part_' + String(nextNum).padStart(3, '0'), events: chunkEvents });
        nextNum++;
      }
    }
    ...
    for (const chunk of chunks) {
      ...
      const name = chunk.id;
      const info = await this._getDataFileInfo(name, this.ARCHIVE_DIR);
      await this._putDataFile(name, payload, info.sha, this.ARCHIVE_DIR);
