...
const BilibanBackupSync = {
  ...
  // 写 <dir>/<name>.json 到数据仓库（dir 默认 data/backup，归档传 data/archive）
  async _putDataFile(name, payload, sha, dir) {
    const repoPath = await this.ensureDataRepo();
    const filePath = (dir || this.BACKUP_DIR) + '/' + name + '.json';
  ...
  // 获取文件信息（含 sha），404 返回 exists:false；dir 默认 data/backup
  async _getDataFileInfo(name, dir) {
    const repoPath = await this.getDataRepoFullName();
    const filePath = (dir || this.BACKUP_DIR) + '/' + name + '.json';
    try {
  ...
  // 读取文件内容（404 返回 null）。dir 指定时只查该目录；否则新路径 backup/ 优先、旧路径 data/ 兜底
  async readDataFile(name, dir) {
    const repoPath = await this.getDataRepoFullName();
    const dirs = dir ? [dir] : [this.BACKUP_DIR, this.DATA_DIR];
    for (const d of dirs) {
      const filePath = d + '/' + name + '.json';
      try {
        const file = await BilibanGithubSync.request('/repos/' + repoPath + '/contents/' + filePath);
  ...
  // 列出云端归档分块（data/archive/part_*.json，含 meta 摘要；兼容旧错位 backup/archive/）
  async listArchiveParts() {
    const repoPath = await this.getDataRepoFullName();
    const dirs = [this.ARCHIVE_DIR, this.BACKUP_DIR + '/archive'];
    const result = [];
    const seen = new Set();
    for (const dir of dirs) {
      ...
      for (const c of contents) {
        ...
        const name = c.name.replace(/\.json$/, '');
        if (seen.has(name)) continue; // 新路径优先，旧路径同名去重
        seen.add(name);
        try {
          const part = await this.readDataFile(name, dir);
          result.push({ name: name, dir: dir, meta: part && part.meta ? part.meta : null, size: c.size });
        } catch (e) { /* 单个分块损坏不影响列表 */ }
      }
    }
  ...
  // 删除文件（dir 默认 data/backup；归档传 data/archive）
  async _deleteFile(name, dir) {
    const repoPath = await this.getDataRepoFullName();
    const filePath = (dir || this.BACKUP_DIR) + '/' + name + '.json';
    const info = await this._getDataFileInfo(name, dir);
    if (!info.exists) return { deleted: false };
    await BilibanGithubSync.request('/repos/' + repoPath + '/contents/' + filePath, {
      method: 'DELETE',
      body: JSON.stringify({ message: 'BILIBAN: 删除 ' + name, sha: info.sha })
    });
    return { deleted: true };
  },

  // 归档未归档数据（半归档机制）：拉取 staging → 合并 → 满阈值切正式分块 → 不足留 staging。返回 { archived, parts, message }
  async archivePendingData() {
    ...
    // 拉取半归档（staging），与新数据合并（按时间排序，旧数据在前）
    const staging = await this.readDataFile('staging', this.ARCHIVE_DIR);
    const stagingEvents = (staging && Array.isArray(staging.events)) ? staging.events : [];
    const merged = stagingEvents.concat(pending).sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));

    // 从旧到新切块：中间块尽量装满（≤ 阈值）→ 正式分块；最后剩余不足阈值 → 留 staging
    const parts = await this.listArchiveParts();
    let nextNum = parts.length + 1;
    const fullChunks = [];   // [{ id, events }] 正式满块
    let rest = [];           // 不足阈值 → 半归档
    let queue = merged.slice();
    while (queue.length > 0) {
      const chunkEvents = [];
      while (queue.length > 0) {
        const ev = queue[0];
        const est = JSON.stringify({ events: chunkEvents.concat(ev) }).length;
        if (chunkEvents.length > 0 && est > thresholdBytes) break;
        chunkEvents.push(ev);
        queue.shift();
      }
      if (chunkEvents.length === 0) break;
      if (queue.length === 0) {
        // 队列耗尽：最后一块若不足阈值 → 留 staging；足阈值 → 正式
        const est = JSON.stringify({ events: chunkEvents }).length;
        if (est >= thresholdBytes) {
          fullChunks.push({ id: 'part_' + String(nextNum).padStart(3, '0'), events: chunkEvents });
          nextNum++;
        } else {
          rest = chunkEvents;
        }
    ...
    for (const chunk of fullChunks) {
      ...
      const meta = {
        ...
        sizeBytes: JSON.stringify({ meta: {}, events: chunk.events }).length
      };
      const payload = { meta: meta, events: chunk.events };
      const info = await this._getDataFileInfo(chunk.id, this.ARCHIVE_DIR);
      await this._putDataFile(chunk.id, payload, info.sha, this.ARCHIVE_DIR);
      uploaded.push({ id: chunk.id, eventCount: chunk.events.length });
    }

    // 半归档处理：有剩余 → 写 staging；无剩余 → 删除 staging（幂等）
    if (rest.length > 0) {
      const ts = rest.map(e => e.timestamp || 0);
      const stagingMeta = {
        id: 'staging',
        version: 2,
        lastMergedAt: Date.now(),
        timeRange: { from: Math.min.apply(null, ts), to: Math.max.apply(null, ts) },
        eventCount: rest.length,
        sizeBytes: JSON.stringify({ meta: {}, events: rest }).length
      };
      const info = await this._getDataFileInfo('staging', this.ARCHIVE_DIR);
      await this._putDataFile('staging', { meta: stagingMeta, events: rest }, info.sha, this.ARCHIVE_DIR);
    } else {
      await this._deleteFile('staging', this.ARCHIVE_DIR);
    }

    // 全部上传成功 → 更新 cursor（指向本次处理的所有事件最后一条，含留 staging 的）
    const lastEv = merged[merged.length - 1];
    ...
    return {
      ...
      parts: uploaded,
      message: '归档完成：新增 ' + pending.length + ' 事件' + (uploaded.length ? '，生成 ' + uploaded.map(u => u.id).join(', ') : '，不足阈值留半归档') + (rest.length ? '，半归档 ' + rest.length + ' 条' : '')
    };
  },
