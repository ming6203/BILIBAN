 ...
 * Data structure:
 * {
 *   groups: [{ id, name, enabled, uids: number[] }],
 *   sources: [{ id, owner, repo, path, branch, name }]
 * }
 */
...
const BilibanStorage = {
  ...
  async addUidToGroup(groupId, uid) {
    const data = await this._load();
    const group = data.groups.find(g => g.id === groupId);
    if (group && !group.uids.includes(uid)) {
      group.uids.push(uid);
      await this._save(data);
      return true;
    }
    ...
    for (const { data: remoteData } of results) {
      for (const remoteGroup of (remoteData.groups || [])) {
        ...
        for (const uid of (remoteGroup.uids || [])) {
          if (!localGroup.uids.includes(uid)) {
            localGroup.uids.push(uid);
            totalMerged++;
          }
        }
      }
